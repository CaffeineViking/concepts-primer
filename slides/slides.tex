...
T mean(const T* begin,
    ...
    \begin{frame}[fragile]{Read the Specification}
    ...
    \begin{frame}[fragile]{Type Traits and SFINAE}
        \framesubtitle{Problems and Solutions}
        \begin{center}
...
struct Factory {
    ...
    \begin{frame}[fragile]{The Ranges Library}
        \begin{center}
        \begin{lstlisting}[caption={example usage of the powerful range composability features.}]
vector<int> nums { 5, 5, 2, 1, 1, 3, 4, 6, 7, 11 };
nums = move(nums) | action::sort | action::uniques;
//          { 1, 2, 3, 4, 5, 6, 7, 11 }
auto even_nums = nums | view::remove_if([](int x) {
                            return x % 2 == 1; })
                      | view::transform([](int x) {
                            return to_string(x);
                      }); // ---> { "2", "4", "6" } \end{lstlisting}
        \end{center}
    \end{frame}
